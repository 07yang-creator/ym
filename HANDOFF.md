...
**`supabase/migrations/0024_ym_join_hardening.sql`（新文件，⚠ 要 owner 去 SQL editor 跑）**：
...
⚠ §2 需要在 0019 的 `ym_join_apply` 里把那三行改成调用 `ym_join_cap_ok()`，**没改之前 §2 不生效**
（文件里写了要改哪几行；我没有重抄整段函数体 —— 抄错的风险比收益大）。

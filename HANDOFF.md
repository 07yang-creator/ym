...
第③条 0010 的 RLS 本来就满足（ym_ok = approved 主办 OR admin）；第②条也不用动
（ym_post_source 唯一索引只钉 source_id 非空的活动派生卡）。真正要动的只有：

- **⚠ 0030_ym_poster.sql（owner 待跑）**：kind 的 CHECK 只认三种，插 'poster' 是 23514。
  没跑之前发布台照常能写草稿界面，但「发布/存草稿」会被服务端拒 —— 前端认得这个错，
  toast 直接点名「请管理员先跑迁移 0030」。跑完自检 raise notice。
- **发布台**：＋海报 排第一；composer 同一台（海报：标题+图必填硬闸，正文/视频可选；
  多页海报传多张=官网自动轮播）。0030 未跑的报错翻成人话。
- **官网**：kind='poster' → 海报卡（图当卡面 + 日期/标题垫底），**有海报整节让位、
  零海报旧文字卡兜底**（过渡期第三期那两张卡不消失）；点开走同一个阅读器；
  卡面 src 先还原 &amp; 再过 safeUrl（库不是信任边界）。没图的海报退回文字卡（兜底）。
- 活动页「公开到官网」没动 —— 有海报时它的卡只是不显示；哪天要撤这个开关是 owner 的另一个决定。

验证：jsdom 整页跑真 index.html（beforeParse 停在 fetch 边界，桩按 PostgREST 的**已排序**
契约回话 —— 第一版桩没排序还当场教育了我一次）7 条全过：替换/兜底/无图退化/点开阅读器/
文章新闻不受牵连。套件 +5 pin。

---

# ym 交接 — 2026-08-07 晚 III（首页认得登录的人）

owner：主办登着号回首页，右上角还是「登录 / 加入」，也没有进主办台的门。
修法**只做显示**：读同源 localStorage 里 supabase 的会话钥匙（三页共用一把），按钮换成
名字（邮箱 @ 前段，textContent 不进 innerHTML），下拉换两扇门（进入主办台 / 我的成员页）。
不装 supabase 的 JS 客户端、不发任何请求 —— 安全边界仍在 app 服务端（CLAUDE §1）。
判「退出」只看钥匙在不在：access_token 过期不算（refresh 在，进 app 自动续）。
角色故意不猜：两扇门都给，走错的那扇有自己的墙。
⚠ 撞了一次规矩 9：注释里写了 read-only pin 禁的那个连字词（supabase 的 js 包名），当场改口。
